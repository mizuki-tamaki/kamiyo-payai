    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      ...
      try {
        ...
        if (keyRecord.status !== 'active') {
          return res.status(401).json({ error: 'API key is revoked or expired' });
        }
        ...
        const tier = keyRecord.user.subscriptions[0]?.tier || 'free';
        const rateLimit = await checkRateLimit(keyRecord.user.id, tier);
        ...
        await recordApiRequest(keyRecord.user.id, url);
        return handler(req, res);
